...
**Live:** [https://relaxed-weasel-zenml.cloud.nexlayer.ai](https://relaxed-weasel-zenml.cloud.nexlayer.ai)  
...
**zenml** is deployed on [Nexlayer](https://nexlayer.ai) — a container-native
...
application:
  ...
  pods:
  - name: app
    image: mirror.gcr.io/zenmldocker/zenml-server:latest
    path: /
    servicePorts:
    - 8080
    vars:
      ZENML_SERVER_DEPLOYMENT_TYPE: local
      DATABASE_URL: "sqlite:////zenml/.zen/config.db"
    volumes:
    - name: zenml-data
      mountPath: /zenml
      size: 5Gi
